...
A simple and elegant To-Do List application built with Flask (Python) and MongoDB.
...
# todo-app Project End to End Implementation
...
> [!Note]
> This project will be implemented on  (us-east-1).
...
- <b id="EKS">Create EKS Cluster on AWS (Master machine)</b>
  ...
  ```bash
  eksctl create cluster --name=todo-app \
                      --region=us-east-1 \
                      --version=1.33 \
                      --without-nodegroup
  ```
  ...
  ```bash
  eksctl create nodegroup --cluster=todo-app \
                       --region=us-east-1 \
                       --name=todo-app \
                       --node-type=t2.large \
                       --nodes=2 \
...
#
- <b>Create a <mark>todo-CI</mark> pipeline</b>

#
- <b>Create one more pipeline <mark>todo-CD</mark></b>
...
> 52.53.156.187:32738 --> This should be your argocd url
  ...
  ```bash
  argocd cluster add Todo-app@todo-app.us-east-1.eksctl.io --name todo-app-eks-cluster
  ```
  > [!Tip]
  > Todo-app@todo-app.us-east-1.eksctl.io --> This should be your EKS Cluster Name.
...
```bash
eksctl delete cluster --name=todo-app --region=us-east-1
```
